...
description: >-
  Create Excalidraw diagram JSON files that make visual arguments. Use PROACTIVELY
  when user says "excalidraw", "create diagram", "draw architecture", "visualize
  this", "make a flowchart", "diagram this workflow", "draw a system diagram",
  "excalidraw file", "architecture diagram", or "visual diagram". Use when user
  wants to visualize workflows, system architectures, or complex concepts.
allowed-tools: Read Write Edit
argument-hint: "[diagram-title] (optional)"
...
- `roughness: 2` — Sketchy, loose feel.

**Default to `roughness: 1` (subtle hand-drawn feel) for all diagrams.** Only use `roughness: 0` if the user explicitly asks for a clean/polished look. Avoid `roughness: 2` — too sketchy.
...
```bash
cd .claude/skills/excalidraw-architect/references && uv run python render_excalidraw.py <path-to-file.excalidraw>
```
...
```bash
cd .claude/skills/excalidraw-architect/references
uv sync
...
## Share & Save (MANDATORY)

After the diagram passes the render loop, **always** do both of these steps before finishing.

### 1. Generate Shareable Web URL

Use the `mcp__claude_ai_Excalidraw__export_to_excalidraw` MCP tool to upload the diagram and get a shareable `excalidraw.com` link. Pass the full file content.

Present the URL to the user like this:
```
Open in Excalidraw: https://excalidraw.com/#json=...
```

### 2. Save to Obsidian Vault

Copy the `.excalidraw` file to the Yasmine-OS Obsidian vault:

**Vault path:** `/Users/yasmineseidu/Library/Mobile Documents/iCloud~md~obsidian/Documents/Yasmine-OS/14-Excalidraw/`

Use the Write tool to write the file content to:
```
/Users/yasmineseidu/Library/Mobile Documents/iCloud~md~obsidian/Documents/Yasmine-OS/14-Excalidraw/<diagram-name>.excalidraw
```

Use the same filename as the source `.excalidraw` file. If the diagram title contains spaces, use kebab-case for the filename (e.g., `lead-research-pipeline.excalidraw`).

**After both steps, deliver to the user:**
1. The shareable web URL
2. Confirmation that the file was saved to `14-Excalidraw/` in the vault
...
18. **Roughness**: `roughness: 1` (subtle hand-drawn) — use `roughness: 0` for clean/polished, never `roughness: 2` (too sketchy)
19b. **Stroke width on shapes**: `strokeWidth: 1` (thin pencil) — avoid `strokeWidth: 2` or higher unless emphasis needed
19c. **Shape default**: `rectangle` with `"roundness": {"type": 3}` for all containers — rounded rectangles, not plain boxes or ellipses
...
### Share & Save (Non-Negotiable)
28. **Web URL generated**: Used `mcp__claude_ai_Excalidraw__export_to_excalidraw` and presented the link
29. **Saved to Obsidian**: File written to `14-Excalidraw/` in Yasmine-OS vault
